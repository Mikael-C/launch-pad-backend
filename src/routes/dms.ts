...
dmsRouter.post('/register', requireAuth, asyncHandler(async (req: AuthRequest, res: Response) => {
  const {
    platform = 'web',
    osVersion,
    serialNumber,
...
// ─── POST /api/dms/attest ─────────────────────────────────────
dmsRouter.post('/attest', requireAuth, asyncHandler(async (req: AuthRequest, res: Response) => {
...
dmsRouter.post('/integrity-report', requireAuth, asyncHandler(async (req: AuthRequest, res: Response) => {
  const {
    deviceId,
    isJailbroken = false,
    isEmulator = false,
  ...
  if (isJailbroken) trustScore = 0;
  else if (hasHooking) trustScore = 0;
  else if (isEmulator) trustScore = Math.min(trustScore, 30);
  else if (hasDebugger) trustScore = Math.min(trustScore, 30);
...
// ─── GET /api/dms/status ──────────────────────────────────────
dmsRouter.get('/status', requireAuth, asyncHandler(async (req: AuthRequest, res: Response) => {
  const userId = req.user!.walletAddress.toLowerCase();
...
// ─── GET /api/dms/devices ─────────────────────────────────────
dmsRouter.get('/devices', requireAuth, asyncHandler(async (req: AuthRequest, res: Response) => {
  const userId = req.user!.walletAddress.toLowerCase();
...
dmsRouter.delete('/devices/:deviceId', requireAuth, asyncHandler(async (req: AuthRequest, res: Response) => {
  const userId = req.user!.walletAddress.toLowerCase();
  const { deviceId } = req.params;
  
  const device = await prisma.registeredDevice.findUnique({ where: { deviceId } });
  if (!device) return res.status(404).json({ error: 'Device not found' });
  if (device.userId !== userId) return res.status(403).json({ error: 'Not your device' });
