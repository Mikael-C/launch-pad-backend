...
import { requireAuth, optionalAuth, AuthRequest, verifyWalletSignature } from '../middleware/auth.js';
import { asyncHandler } from '../middleware/errorHandler.js';

export const part2Router = Router();

// ─── GET /api/marketplace/projects ───────────────────────────
part2Router.get('/projects', asyncHandler(async (req, res: Response) => {
  const {
    chain, category, tier, status = 'approved',
  ...
  const where: any = {};
  if (status) where.status = { in: status === 'all' ? ['approved', 'active', 'completed'] : [status as string] };
...
part2Router.get('/projects/:id/stats', asyncHandler(async (req, res: Response) => {
  const { id } = req.params;
  ...
  const tvlChart = investments.reduce((acc: any[], inv) => {
...
part2Router.get('/watchlist', requireAuth, asyncHandler(async (req: AuthRequest, res: Response) => {
...
part2Router.post('/watchlist', requireAuth, asyncHandler(async (req: AuthRequest, res: Response) => {
...
part2Router.delete('/watchlist/:projectId', requireAuth, asyncHandler(async (req: AuthRequest, res: Response) => {
  const sxId = req.user!.walletAddress.toLowerCase();
...
part2Router.get('/comments', asyncHandler(async (req, res: Response) => {
...
part2Router.post('/comments', requireAuth, asyncHandler(async (req: AuthRequest, res: Response) => {
...
part2Router.post('/upvote', requireAuth, asyncHandler(async (req: AuthRequest, res: Response) => {
  const { projectId } = req.body;
  const sxId = req.user!.walletAddress.toLowerCase();
