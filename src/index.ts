...
app.get('/api/referral/stream', (req, res) => {
  ...
  const sendUpdate = async () => {
    try {
      ...
      const successful = referrals.filter((r: { status: string }) => r.status === 'completed').length;
      const pending = referrals.filter((r: { status: string }) => r.status === 'pending').length;
      const rewards = successful * 100;
      const byPlatform = {
        telegram: referrals.filter((r: { platform: string }) => r.platform === 'telegram').length,
        twitter: referrals.filter((r: { platform: string }) => r.platform === 'twitter').length,
        facebook: referrals.filter((r: { platform: string }) => r.platform === 'facebook').length,
      };
