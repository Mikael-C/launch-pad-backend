...
export interface DPoPPayload {
  ...
  htu: string;    // HTTP URI
  iat: number;
  ath: string;    // Access token hash (SHA-256)
}

export const validateDPoP = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const dpopDecoded = jwt.decode(dpopHeader, { complete: true }) as any;
    if (!dpopDecoded) {
...
export const requireDPoP = async (req: Request, res: Response, next: NextFunction) => {
  const dpopHeader = req.headers['dpop'] as string;
