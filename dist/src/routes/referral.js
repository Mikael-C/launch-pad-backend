...
referralRouter.get('/links', requireAuth, asyncHandler(async (req, res) => {
...
referralRouter.post('/links/generate', requireAuth, asyncHandler(async (req, res) => {
...
referralRouter.get('/stats', requireAuth, asyncHandler(async (req, res) => {
    const referrerId = req.user.walletAddress.toLowerCase();
    const referrals = await prisma.referral.findMany({
        where: { referrerId },
...
referralRouter.post('/track', asyncHandler(async (req, res) => {
    const { code } = req.body;
...
referralRouter.get('/validate', asyncHandler(async (req, res) => {
...
referralRouter.post('/register', requireAuth, asyncHandler(async (req, res) => {
    const { code } = req.body;
...
referralRouter.post('/complete', requireAuth, asyncHandler(async (req, res) => {
...
referralRouter.post('/simulate', requireAuth, asyncHandler(async (req, res) => {
    const { count = 5, platform = 'twitter' } = req.body;
    const referrerId = req.user.walletAddress.toLowerCase();
    if (count > 20)
        return res.status(400).json({ error: 'Max 20 simulations at once' });
    const created = [];
