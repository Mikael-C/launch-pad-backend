...
import { requireAuth, requireSuperAdmin, verifyWalletSignature } from '../middleware/auth.js';
...
// ─── GET /api/projects ────────────────────────────────────────
part1Router.get('/projects', asyncHandler(async (req, res) => {
    const { status, chain, category, tier, page = '1', limit = '10', sortBy = 'createdAt', order = 'desc' } = req.query;
    const where = {};
    if (status)
        where.status = status;
    if (chain)
...
// ─── GET /api/projects/:id ────────────────────────────────────
part1Router.get('/projects/:id', asyncHandler(async (req, res) => {
...
// ─── POST /api/projects ───────────────────────────────────────
part1Router.post('/projects', requireAuth, asyncHandler(async (req, res) => {
...
part1Router.put('/projects/:id/approve', requireAuth, requireSuperAdmin, asyncHandler(async (req, res) => {
    const { id } = req.params;
...
part1Router.post('/whitelist/add', requireAuth, asyncHandler(async (req, res) => {
...
part1Router.get('/whitelist/check', asyncHandler(async (req, res) => {
    ...
    const invested = entry ? await prisma.investment.aggregate({
        where: { userId: walletAddress.toLowerCase(), projectId: projectId },
        _sum: { amount: true }
    }) : null;
    res.json({
        whitelisted: !!entry,
...
// ─── POST /api/invest ─────────────────────────────────────────
part1Router.post('/invest', requireAuth, asyncHandler(async (req, res) => {
...
part1Router.get('/vesting/claimable', requireAuth, asyncHandler(async (req, res) => {
    const walletAddress = req.user.walletAddress.toLowerCase();
    const { projectId } = req.query;
    const where = { userId: walletAddress };
    if (projectId)
...
part1Router.post('/vesting/claim', requireAuth, asyncHandler(async (req, res) => {
    ...
    if (!schedule)
        return res.status(404).json({ error: 'No vesting schedule found' });
    const now = new Date();
...
// ─── GET /api/investments ─────────────────────────────────────
part1Router.get('/investments', requireAuth, asyncHandler(async (req, res) => {
    const walletAddress = req.user.walletAddress.toLowerCase();
