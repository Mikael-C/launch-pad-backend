...
adminRouter.get('/super-admins', requireAuth, requireSuperAdmin, asyncHandler(async (_req, res) => {
...
adminRouter.post('/super-admins', requireAuth, requireSuperAdmin, asyncHandler(async (req, res) => {
...
adminRouter.post('/operations/propose', requireAuth, requireSuperAdmin, asyncHandler(async (req, res) => {
...
adminRouter.post('/operations/:id/approve', requireAuth, requireSuperAdmin, asyncHandler(async (req, res) => {
    const { id } = req.params;
    ...
    if (operation.status !== 'pending')
        return res.status(400).json({ error: `Operation is ${operation.status}` });
    ...
    if (existingSigs.some((s) => s.adminWallet === adminWallet)) {
...
adminRouter.post('/operations/:id/execute', requireAuth, requireSuperAdmin, asyncHandler(async (req, res) => {
    const { id } = req.params;
    const operation = await prisma.pendingOperation.findUnique({ where: { id } });
    if (!operation)
        return res.status(404).json({ error: 'Operation not found' });
...
adminRouter.get('/operations', requireAuth, requireSuperAdmin, asyncHandler(async (_req, res) => {
...
adminRouter.post('/kill-switch/activate', requireAuth, requireSuperAdmin, asyncHandler(async (req, res) => {
...
adminRouter.post('/kill-switch/deactivate', requireAuth, requireSuperAdmin, asyncHandler(async (req, res) => {
...
adminRouter.get('/kill-switch/status', asyncHandler(async (_req, res) => {
...
adminRouter.get('/contracts', asyncHandler(async (_req, res) => {
    res.json({
        baseSepolia: {
...
adminRouter.get('/dashboard', requireAuth, requireSuperAdmin, asyncHandler(async (_req, res) => {
    const [totalProjects, pendingProjects, totalInvestments, totalUsers, deviceStats, killSwitch, pendingOps] = await Promise.all([
